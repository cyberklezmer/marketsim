	...
	private:
		...
		{	
			...
			if (!lastresult)
			{
				...
				bnd_ask = 10000, bnd_m = 10000; //change to sth dynamic
						...
						for (int f = round(F_mean - 4 * F_stddev); f <= round(F_mean + 4 * F_stddev); f++)
						{
							...
							{ 
								...
								for (int y = 0; y <= 1; y++)
								{
									...
									int ind_ask = std::min(std::max(0, a + f), bnd_ask);

									expv += v + discfact * W[ind_m][ind_ask] * prob;

									ind_m = std::min(
										std::max<int>(0, m + y * k_m * c - v_best * (a + round(coefs[0] + coefs[1] * (v_best - 1) + e))),
										bnd_m);

									expv_opt += v_best + discfact * W[ind_m][ind_ask] * prob;
								}	
							}
						}
					}
